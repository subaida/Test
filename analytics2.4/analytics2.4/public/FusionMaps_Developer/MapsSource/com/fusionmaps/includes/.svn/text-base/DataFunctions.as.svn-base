...
*/
/**
 * isDataURLProvided function checks whether the map has been 
...
function isDataURLProvided():Boolean {
	...
	if (rootAttr["dataurl"] == "" || rootAttr["dataurl"] == null || rootAttr["dataurl"] == undefined) {
		...
		if (rootAttr["dataxml"] == "" || rootAttr["dataxml"] == null || rootAttr["dataxml"] == undefined) {
			...
			//Add to log
			map.log("WARNING", "Could not find dataXML or dataURL parameter. Setting dataURL to default data file "+_defaultDataFile+".", Logger.LEVEL.ERROR);
		} else {
			//We have been provided with the full XML document
			//So, re-set the flag
			isURLProvided = false;
			//Add to log
			map.log("INFO", "XML Data provided using dataXML method.", Logger.LEVEL.INFO);
		}
	} else {
		//Add to log
		map.log("INFO", "XML Data provided using dataURL method.", Logger.LEVEL.INFO);
	}
	return isURLProvided;
}
/**
 * Method to check whether the previous data was provided using
 * dataURL method. This is called from resize() method when the map
 * is resized from JavaScript. It silently returns the value without
 * logging anything.
 * @return		Whether the chart is using dataURL method.
*/
function isDataURLProvidedSilent():Boolean{
	return (rootAttr ["dataxml"] == "" || rootAttr ["dataxml"] == null || rootAttr ["dataxml"] == undefined);
}
/** 
 ...
 * 1. Convert the URL Encoded dataURL back to normal form.
 * 2. Create the no-cache form of the URL
*/
...
/**
 * loadData function loads the data for the map and stores 
...
function loadData() {
	...
	if (isDataURLProvided()) {
		...
		if (_isSafeDataURL){
			map.log ("dataURL provided", "<A HREF='" + rootAttr ["dataurl"] + "' target='_blank'>" + rootAttr ["dataurl"] + "</A>", Logger.LEVEL.LINK);
		}else{
			map.log ("dataURL reset", "A colon character was found in dataURL, which can be potentially dangerous as it allows XSS attacks. Re-setting dataURL to Data.xml. If you're using absolute URLs (like http://domain.com/...) to provide dataURL, please convert it to relative path for increased security.", Logger.LEVEL.ERROR);
		}
		//Get the filtered data URL
		var strURL:String = getFilteredDataURL(rootAttr["dataurl"]);
		//Add filtered URL to log
		map.log("dataURL invoked", "<A HREF='"+strURL+"' target='_blank'>"+strURL+"</A>", Logger.LEVEL.LINK);
		...
		tf = createBasicText(getAppMessage("RETRIEVINGDATA", _lang), this, 2, _mapXCenter, _mapYCenter+5, "Verdana", 10, LPBarTextColor, "center", "bottom");
		...
		tf = createBasicText(getAppMessage("READINGDATA", _lang), this, 2, _mapXCenter, _mapYCenter, "Verdana", 10, LPBarTextColor, "center", "middle");
...
function dataLoaded(success:Boolean):Void {
	...
	if (success) {
		...
		//TODO: Debug this line. Playhead not going to DataLoad,FDataLoadDone when map laoded with this sequence: 1. Normal data.xml, 2. Error data using setDataXML, 3. setDataURL to a valid XML
		gotoAndPlay("DataLoad", "FDataLoadDone");
	} else {
		//Delete the enter frame event					
		delete this.onEnterFrame;
		//An error occurred while fetching the data. Show an error to viewer
		tf = createBasicText(getAppMessage("LOADERROR", _lang), this, 2, _mapXCenter, _mapYCenter, "Verdana", 10, LPBarTextColor, "center", "middle");
		//Add to log.
		map.log("ERROR", "An error occurred while loading data. Please check your dataURL, by clicking on the \"dataURL invoked\" link above, to see if it's returing valid XML data. Common causes for error are: <LI>No URL Encoding provided for querystrings in dataURL. If your dataURL contains querystrings as parameters, you'll need to URL Encode the same. e.g., Data.asp?id=101&subId=242 should be Data%2Easp%3Fid%3D101%26subId%3D242</LI><LI>Different sub-domain of map .swf and dataURL. Both need to be same owing to sandbox security.</LI><LI>Network error</LI>", Logger.LEVEL.ERROR);
		//Stop
		stop();
	}
}
 ...
 * If the map is not working in local mode, we will add a continuously 
 ...
 * script method setDataURL. This method updates the map
 * by loading the new data and re-rendering the map.
 *	@param	strURL	New URL from where data is to be 
 *					loaded.
*/
function setDataURL(strURL:String):Void {
	//This function re-sets the dataURL for the map.
	...
	map.reInit();
	//Log the event
	map.log("INFO", "setDataURL method invoked from external script.", Logger.LEVEL.INFO);
	//Call loadData
	loadData();
}
/**
 * setDataXML method interfaces with the external
 * script method setDataXML. This method updates the map
 * by reading the new data and re-rendering the map.
 *	@param	strXML	New XML Data.
*/
function setDataXML(strXML:String):Void {
	//This function re-sets the dataXML for the map.
	//First we need to make sure that any previous calls are not running
	deletePendingCalls();
	//Now set XML
	...
	map.reInit();
	//Log the event
	map.log("INFO", "setDataXML method invoked from external script.", Logger.LEVEL.INFO);
	//Call loadData
	loadData();
}
/**
 * resizeMap method interfaces with the external script method resize.
 * This method resizes the map keeping the existing XML data by reading 
 * the new dimensions and re-rendering the chart.
 * Here, we basically keep the same map instance, remove it, reInit it,
 * use the same XML data, re-parse it and re-draw the map. We also ask the
 * map not to animate again.
 *	@param	strXML	New XML Data.
*/
function resizeMap (intWidth:Number, intHeight:Number) : Void {
	//Convert to numeric representations
	intWidth = Number(intWidth);
	intHeight = Number(intHeight);
	//If width and height are non numeric, return
	if (isNaN(intWidth) || isNaN(intHeight)){
		return;
	}else if(intWidth<1 || intHeight<1){
		//New width and height cannot be less than 1 too.
		return;
	}
	//Note: Global size variables do not need to be updated, as they're never referred to
	//after instantiation of a map instance.
	//If dataURL method was used and previous XML has not yet loaded, just
	//setting the size would do the trick. 
	if ((isDataURLProvidedSilent() && xmlData.loaded) || !(isDataURLProvidedSilent())){
		//We're checking for XML's loaded property here, because it can happen that the user invokes resize
		//method before the previous XML has finished loading. We check only in case of dataURL.		
		//Remove the existing map
		map.remove ();
		//Re-initialize
		map.reInit ();
		//Set the new size
		map.setSize (intWidth, intHeight);
		//Set the new debug window size
		map.updateLogSize();
		//Set animation false to false
		map.setAnimationFlag (false);
		//Log the event
		map.log ("INFO", "resize method invoked from external script. Setting new size to " + String(intWidth) + " x " + String(intHeight) + " pixels", Logger.LEVEL.INFO);
		//Set the debug mode of map to false, as we do not want to log the XML again
		map.setDebugMode(false);
		//No need to reset/reload XML, as XML is already stored in xmlData root attribute		
		//Call for rendering again	
		gotoAndPlay ("DataLoad", "FDataLoadDone");
	}else{
		//The control comes here if the previous XML is still loading. So, we just update the
		//size of chart.
		map.setSize (intWidth, intHeight);
	}
}
