 ...
 * 
 * The code in this file pre-loads the map and after loading sends the 
...
Variables that the user can specify in the map container:
...
the progress bar elements with respect to the map size. So,
the width of the progress bar is set in such a way that is the 
map width is greater than 200 pixels, the progress bar width
will be 150 pixels. However, if the map width is less than
200 pixels, then the progress bar width will be 25 pixels less
than the map width.
*/
LPBarWidth = (_mapWidth>200) ? 150 : (_mapWidth-25);
...
LPBarLoadingText = unescape(getFirstValue(rootAttr["pbarloadingtext"], getAppMessage("LOADINGMAP", _lang)));
/**
...
var pBXPos:Number = _mapXCenter-(LPBarWidth/2);
var pBYPos:Number = _mapYCenter-LPBarHeight;
...
if (getBytesLoaded()>=getBytesTotal()) {
	...
	*/
	...
	var tfLoad:TextField = createBasicText(LPBarLoadingText, this, 2, _mapXCenter, _mapYCenter+5, "Verdana", 10, LPBarTextColor, "center", "bottom");
	//Create the preloader sequence
	this.onEnterFrame = function() {
		if (getBytesLoaded()<getBytesTotal()) {
			//Means, the map hasn't still loaded
