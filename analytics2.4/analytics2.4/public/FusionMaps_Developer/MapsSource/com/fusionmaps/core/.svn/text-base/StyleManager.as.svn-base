 ...
 * StyleManager class helps each map manage its own list of styles
 * and apply them to individual objects of the map.
 ...
 *							map object. The key is the object
...
import com.fusionmaps.helper.HashTable;
//Import filter classes
import flash.filters.BlurFilter;
import flash.filters.DropShadowFilter;
import flash.filters.BevelFilter;
import flash.filters.GlowFilter;
class com.fusionmaps.core.StyleManager {
	//Create enumeration for the various style type that's supported
	public var TYPE:FCEnum;
	//styles HashTable stores all the style defintion for the map
	...
	//Map class which is associated with this style manager
	private var mapIns:Map;
	//Constructor function
	function StyleManager(mapIns:Map) {
		//Store map instance
		this.mapIns = mapIns;
	...
	/**
	 * addStyle method defines a style method for this map.
	...
	*/
	...
	/**
	 * associateStyle method associates a style with a map object.
	 *	@param	mapObject		Numeric index of map object for which
	 *							we're defining style
	 *	@param	strStyleName	Name of style which we're associating.
	*/
	public function associateStyle(mapObject:Number, strStyleName:String):Boolean {
		//Remove any spaces from style name
		strStyleName = StringExt.replace(strStyleName, " ", "");
		...
		if (!isValidStyle(strStyleName)) {
			//Throw error
			throw new FCError("Invalid Style Name in Style Definition", "Style \""+strStyleName+"\" has not been defined and as such cannot be associated with a map object.", Logger.LEVEL.ERROR);
			return false;
		}
		//Get the current list of styles defined for this object                                              
		var currentStyles:Array;
		currentStyles = this.objectStyle[mapObject];
		if (currentStyles == undefined) {
			//Create a new array to store styles for this object
			this.objectStyle[mapObject] = new Array(strStyleName);
		} else {
			//We iterate through the array to find, whether this style
			//has already been associated with this map object
			...
			if (!associated) {
				this.objectStyle[mapObject].push(strStyleName);
			}
		}
	}
	/**
	 ...
	 *	@param		mapObject		map object for which we've to update.
	...
	*/
	private function overrideAssociation(mapObject:Number, strOldStyleName:String, strStyleName:String):Void {
		//Get the current list of styles defined for this object                      
		var currentStyles:Array;
		currentStyles = this.objectStyle[mapObject];
		...
		this.objectStyle[mapObject] = currentStyles;
	}
	/**
	...
	*/
	 ...
	 *	@param	mapObject		Numeric id of the map object for which 
	 *							we need the style
	...
	*/
	public function getStyle(mapObject:Number, intStyleType:Number, animationParam:String):StyleObject {
		//Get the list of styles applicable for this object		
		var objStyles:Array = new Array();
		objStyles = this.objectStyle[mapObject];
		...
		if (intStyleType == this.TYPE.ANIMATION) {
			...
			//First get all animation style objects for this map object
			//Storage container
	 ...
	 * @param	mapObject		Numerical id of the map object for which
	 *							we're working on style details.
	 * @param	
	 * @return					The final style object
	*/
	public function overrideStyle(mapObject:Number, defaultObject:StyleObject, intStyleType:Number, animationParam:String):StyleObject {
		//First, get the style for this object
		var objectStyle:StyleObject = new StyleObject();
		objectStyle = this.getStyle(mapObject, intStyleType, animationParam);
		...
		if (objectStyle.isDefined == true) {
			...
			* One style definition can be associated with multiple objects in the map.
			...
			* the association of the map object from old style name (user defined) to
			* the new one.
			*/
			...
			//associated with this map object to the newly created one.
			this.overrideAssociation(mapObject, objectStyle.name, styleName);
			//Return
			return objectStyle;
		} else {
			//First, check if a name has been defined for this style. If not
			...
			//And, we associate it with the given map object
			this.associateStyle(mapObject, styleName);
			//Return 
	...
	*/
	 ...
	 * @param	mapObject		Numeric id of the map object, for which we've 
	...
	*/
	public function getTextStyle(mapObject:Number):Object {
		//Get the style properties of this object
		var objStyle:StyleObject = new StyleObject();
		objStyle = getStyle(mapObject, this.TYPE.FONT, "");
	 ...
	 * for the list of map objects specified. Each map object can have multiple
	 * animation sequences. So, we first get the maximum animation time for all 
	 * animation defined for a map object. Then we compare it against the other
	 * map objects.
	 *	@param	List of map objects separated by comma.
	 *	@return	Maximum animation time required for any animation style
	 *				for the given map objects.
	*/
	public function getMaxAnimationTime():Number{
		//Initialize variables
		var time:Number = 0;
		var totalMaxTime:Number=0
		var mapObjMaxTime:Number = 0;
		//Array to store the styles for a particular object
		var objStyles:Array
		//Iterate through all the parameters (map object ids) provided
		...
		for (i=0; i<arguments.length; i++){
			...
			for (j=0; j<objStyles.length; j++) {
				...
				if (this.styleType[objStyles[j]] == this.TYPE.ANIMATION) {
					...
					mapObjMaxTime = (time>mapObjMaxTime)?time:mapObjMaxTime;
				}
			}
			//Compare for the maximum among different map objects
			totalMaxTime = (mapObjMaxTime>totalMaxTime)?mapObjMaxTime:totalMaxTime;
		}
	 ...
	 * from the style repository, based on the numeric id of the map
	 * object which is being represented by this movie clip
	 *	@param	mc			Movie clip to which we've to apply animation
	 *	@param	mapObject	The numeric id of map object which is being 
	 *						represented by this movie clip
	*/
	public function applyAnimation(mc:MovieClip, mapObject:Number, dX:Number, dY:Number, dAlpha:Number, dXScale:Number, dYScale:Number, dRotation:Number):Void {
		//First up, we need to get a list of animation styles defined for this map object.
		//Get the list of styles applicable for this object		
		var objStyles:Array = new Array();
		objStyles = this.objectStyle[mapObject];
		var i:Number;
		...
		if (animationStyles.length>0) {
			...
			for (i=0; i<animationStyles.length; i++) {
				...
				//So, suppose a particular map object can support only XScale and Alpha
				...
				switch (animationStyles[i].param.toLowerCase())
				{
					case "_x":
						...
						start = Number(animationStyles[i].start)
						break;					
					case "_y":
						end = dY;					
						start = Number(animationStyles[i].start);
	 ...
	 *						filter type.
	*/
	public function applyFilters(mc:MovieClip, mapObject:Number, arrNotApply:Array):Void {
		//First up, we need to get a list of filters defined for this map object.
		//Get the list of styles applicable for this object		
		var objStyles:Array = new Array();
		objStyles = this.objectStyle[mapObject];
		var i:Number;
		//First get all filter style objects for this map object
		...
		if (filterStyles.length>0) {
			...
			for (i=0; i<filterStyles.length; i++) {
				...
				case this.TYPE.SHADOW :
					 ...
					 *			The default value is 4 (floating point).
					 ...
					 *			format RRGGBB. The default value is CCCCCC.
					...
					*/
					...
					var distance:Number = Number(Utils.getFirstValue(filterStyles[i].distance, 5));
