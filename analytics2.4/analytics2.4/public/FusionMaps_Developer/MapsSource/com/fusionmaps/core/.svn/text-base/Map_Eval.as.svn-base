...
class com.fusionmaps.core.Map
{
	//Instance properties
	//Version of the map.
	private var _version : String = "3.0.5";
	...
	*/
	private function drawBackground () : Void
	{
		...
		// ----------------- EVALUATION MARK -----------------//
		//Just add an evaluation mark to all the charts. We call addEvalMark from
		//here so that we just need to remove from this class and recompile
		this.addEvalMark();		
	}
	/**
	* This function adds the evaluation mark to all the maps
	*/
	private function addEvalMark () : Void
	{
		//Get URL of movie
		var chartURL : String = _root._url;
		chartURL = chartURL.toUpperCase ();
		//Check for by-pass condition
		if (chartURL.indexOf ("INFOSOFTGLOBAL.COM") == - 1 && chartURL.indexOf ("FUSIONCHARTS.COM") == - 1)
		{
			//Create a font object
			var fontObj = new Object ();
			fontObj.align = "left";
			fontObj.vAlign = "bottom";
			fontObj.bold = true;
			fontObj.italic = false;
			fontObj.underline = false;
			fontObj.font = "Verdana";
			fontObj.size = 11
			fontObj.color = "333333";
			fontObj.isHTML = true;
			fontObj.leftMargin = 5;
			fontObj.letterSpacing = 0;
			fontObj.bgColor = "FFFFFF"
			fontObj.borderColor = "333333";
			var captionObj : Object = createText (false, "<a href='http://www.fusioncharts.com/maps'>FusionMaps v3 Evaluation - An InfoSoft Global Creation</a>&nbsp;", parentMC, parentMC.getNextHighestDepth () , 0, 0, 0, fontObj, false, 0, 0);
		}
	}
	/**
	...
	*/
	private function drawLabels () : Void
	{
		...
		for (i = 1; i <= this.tNum; i ++)
		{
			...
			if ((this.entity [i].displayValue != "") && (this.entity [i].showLabel!=false))
			{				
