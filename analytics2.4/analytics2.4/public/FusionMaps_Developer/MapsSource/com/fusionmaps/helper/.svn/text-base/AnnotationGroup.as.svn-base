 ...
 * TODO: How would an annotation group respond to roll over 
 * and click (tool tip and link invoking)? Will it bubble the event
 * up to its parent class?
*/
...
class com.fusionmaps.helper.AnnotationGroup{
	 ...
	 *						this parameter specifies the shift. 
	*/
	function AnnotationGroup(mc:MovieClip, x:Number, y:Number, xScale:Number, yScale:Number, alpha:Number, stageW:Number, stageH:Number, origW:Number, origH:Number, autoScale:Boolean, constrainedScale:Boolean, scaleImages:Boolean, scaleText:Boolean, xShift:Number, yShift:Number){
		//Short forms for common function names.
	...
	public function addItem(itemNode:XMLNode):Void{
		...
		if (obj.type==-1){
			...
			if (obj.type == this.TYPE.TEXT){
				...
				obj.letterMargin = getFN(atts["lettermargin"], 0);
	...
	*/
	private function calculateScale():Void{
		//Scaling for manipulation of items position. Do scaling only if autoScale
		//is on.
		if (this.autoScale){
		...
		for (i=1; i<=this.num; i++){
			...
			switch (this.item[i].type){
				case this.TYPE.TEXT:					
					...
					Utils.createText(false, this.item[i].label, itemMC, itemMC.getNextHighestDepth(), x, y, 0, textStyle, this.item[i].wrap, this.item[i].wrapWidth, this.item[i].wrapHeight);
					break;
				case this.TYPE.LINE:
				...
				case this.TYPE.ARC:
					...
					itemMC.lineStyle();
					itemMC.beginGradientFill(this.item[i].fillPattern, this.item[i].fillColor, this.item[i].fillAlpha, this.item[i].fillRatio, {matrixType:"box", x:-radius, y:-radius, w:radius*2, h:radius*2, r:((360-this.item[i].fillAngle)/180)*Math.PI});
					//DrawingExt.drawArc(itemMC, 0, 0, this.item[i].startAngle, this.item[i].endAngle-this.item[i].startAngle, radius, radius)
					DrawingExt.drawCircle(itemMC, 0, 0, radius, radius, this.item[i].startAngle, this.item[i].endAngle-this.item[i].startAngle);
					itemMC.lineTo(ax , ay);
					//DrawingExt.drawArc(itemMC, 0, 0, this.item[i].endAngle, -(this.item[i].endAngle-this.item[i].startAngle), innerRadius, innerRadius);
