                  ...
                  if @aes.checkSecurity(session[:user_id],session[:key],session[:iv],5,0)==0
                      #initialize start and end date
                       ...
                       session[:pub_id]=@publisher_id[0].id
                        ...
                        if @analytic=="refresh"
                             redirect_to :action=>'generate_report',:adclient_dropdown=>params[:site_id]!=nil ? params[:site_id] : @aes.encrypt(session[:key],session[:iv],'WAP'),:pub_start_on=>@startdate,:pub_end_on=>@enddate,:duration=>(params[:duration]!=nil && params[:duration]!='') ? params[:duration].to_s : '1',:filled=>'filled'
                        end
                       @adclient_summary= @@obj.performance_summary(session[:pub_id],session[:report_adclient]!=nil ? session[:report_adclient] : 'WAP' ,session[:report_start_on]!=nil ? session[:report_start_on] : @startdate ,session[:report_end_on]!=nil ? session[:report_end_on] : @enddate,session[:filled])
              ...
              if (session[:user_id]==nil)
                  ...
                  if @aes.checkSecurity(session[:user_id],session[:key],session[:iv],5,0)==0
                       # @handset_report = session[:xml_string]
                        ...
                        @XML_handset_report=@@obj.generate_query("#{session[:pub_id]}-#{session[:filled]}",'pub_handsets','Pie_chart','handset','impressions',session[:report_adclient],session[:report_start_on],session[:report_end_on],@smartLable,'publisher_analytic','')
                        #initialize start and end date
      ...
      def publisher_channel
             begin
                  ...
                  if (session[:user_id]==nil)
                      ...
                      if @aes.checkSecurity(session[:user_id],session[:key],session[:iv],5,0)==0
                          @smartLable='1' #1 represent smartLable as Enabled
                          @XML_channel_report=@@obj.generate_query("#{session[:pub_id]}-#{session[:filled]}",'pub_channels','Pie_chart','channel','requests',session[:report_adclient],session[:report_start_on], session[:report_end_on],@smartLable,'publisher_analytic','')
                          ...
                          #following code is used to retrive recordset from database
                          @channeldata_pages, @channeldata = paginate_by_sql PubChannel,query,10
                      else
                            flash[:notice]="Your are not an authorized user. Please login with different username."
                            redirect_to :controller=>'login', :action=>'login'
                      end
             ...
             @aes = AESSecurity.new() #creating an object for AES Security
              if (session[:user_id]==nil)
                  ...
                  redirect_to :controller=>'login', :action=>'login'
                    ...
                    if @aes.checkSecurity(session[:user_id],session[:key],session[:iv],5,0)==0
                        @smartLable='1' #1 represent smartLable as Enabled
                        @XML_carrier_report=@@obj.generate_query("#{session[:pub_id]}-#{session[:filled]}",'pub_carriers','Pie_chart','operator','requests',session[:report_adclient],session[:report_start_on], session[:report_end_on],@smartLable,'publisher_analytic','')
                        ...
                        sql="select carriers as operator ,#{session[:filled]=='filled' ? "sum(requests)" : session[:filled]=='unfilled' ? "sum(fill_rate)" : "(sum(requests)+sum(fill_rate))"} as sum_carrier,(#{session[:filled]=='filled' ? "sum(requests)" : session[:filled]=='unfilled' ? "sum(fill_rate)" : "(sum(requests)+sum(fill_rate))"}/(select #{session[:filled]=='filled' ? "sum(requests)" : session[:filled]=='unfilled' ? "sum(fill_rate)" : "(sum(requests)+sum(fill_rate))"} from pub_carriers where pub_id=#{session[:pub_id]}  #{@adclient_type}  and delivery_date>='#{@startdate} 00:00:00' and delivery_date<='#{@enddate} 23:59:59' #{filter_column})*100) as carr_percentage from pub_carriers where pub_id=#{session[:pub_id]} and delivery_date>='#{@startdate} 00:00:00' and delivery_date<='#{@enddate} 23:59:59' #{filter_column} #{@adclient_type} group by operator order by sum_carrier desc"                
          ...
          begin
              ...
              if (session[:user_id]==nil)
                  ...
                  if @aes.checkSecurity(session[:user_id],session[:key],session[:iv],5,0)==0
                      ...
                      @page= params[:page]
                      verifyDBconnection
                      @xml_unique_visitors=@@obj.generate_query("#{session[:pub_id]}-#{session[:filled]}",'adclients_summaries','Visitors','delivery_date','requests_unique_visitors',session[:report_adclient],session[:report_start_on], session[:report_end_on],session[:duration],'publisher_analytic','')
                      @adclient_type=(session[:report_adclient]!='SMS' and session[:report_adclient]!='WAP')? " and client_id=#{session[:report_adclient]}" : " and (ad_client_type='#{session[:report_adclient].upcase}' or ad_client_type='#{session[:report_adclient].downcase}')"
                      ...
                      @visitordata_pages, @visitordata = paginate_by_sql AdclientsSummary,query,10
                 else
                    flash[:notice]="Your are not an authorized user. Please login with different username."
                    redirect_to :controller=>'login', :action=>'login'
                end
            ...
            if (session[:user_id]==nil)
                ...
                if @aes.checkSecurity(session[:user_id],session[:key],session[:iv],5,0)==0
                     ...
                     #calls generate query method to construct DEMOGRAPHIC keyword and xml
                      ...
                      xml_handset_report=@@obj.generate_query("#{session[:pub_id]}-#{session[:filled]}",'pub_handsets','Pie_chart','handset','impressions',session[:report_adclient],params[:pub_start_on], params[:pub_end_on],smartLable,'publisher_analytic','')  if adclient_type!='SMS'
                      ...
                      xml_carrier_report=@@obj.generate_query("#{session[:pub_id]}-#{session[:filled]}",'pub_carriers','Pie_chart','operator','requests',session[:report_adclient],params[:pub_start_on], params[:pub_end_on],smartLable,'publisher_analytic','')
                       ...
                       xml_traffic_visitor=@@obj.generate_query("#{session[:pub_id]}-#{session[:filled]}",'adclients_summaries','Visitors','delivery_date','requests_unique_visitors',session[:report_adclient],params[:pub_start_on], params[:pub_end_on],params[:duration],'publisher_analytic','')
