            ...
            if (session[:user_id]==nil)
                ...
                if @aes.checkSecurity(session[:user_id],session[:key],session[:iv],1,0)==0
                    ...
                    if params[:update]
                        ...
                        @adminXML_overall_report=@@objXmlBuilder.generate_query("campaigns_summaries-#{session[:duration]}",'Overall_Performance','campaign_id',@adminreport_name,session[:adminstart_on],session[:adminend_on],session[:order],session[:limit],'campaign_name')
                        ...
                        @adminXML_summary_report=@@objXmlBuilder.generate_query("campaigns_summaries-#{session[:duration]}",'Campaigns_Performance','campaign_id',@adminreport_name,session[:adminstart_on],session[:adminend_on],session[:order],session[:limit],'campaign_name',((params[:page]!=nil) ? ((params[:page].to_i*session[:limit].to_i)-session[:limit].to_i) : 0 ))
                        puts"_______________test1__________________"
                        puts @adminXML_summary_report.inspect
                        ...
                        if @campaignId== nil or @campaignId=='All'
                            # For Table.
                             puts "-----------@adminreport_name--#{@adminreport_name}-----"
                             @strSql="select campaign_id, c.campaign_name, sum(clicks) as clicks , sum(impressions) as impressions ,sum(cs.amount_spent) as amt_spent,((sum(clicks)/sum(impressions))*100) as ctr, (((#{@adminreport_type})/(select (#{@adminreport_type}) as #{@adminreport_name} from campaigns_summaries cs where delivery_date>='#{(Time.parse(session[:adminstart_on])).strftime("%Y-%m-%d")} 00:00:00' and delivery_date<='#{(Time.parse(session[:adminend_on])).strftime("%Y-%m-%d")} 23:59:59' and (cs.impressions>0 or cs.clicks>0) ))*100) as percentage from campaigns_summaries cs,campaigns c where delivery_date>='#{(Time.parse(session[:adminstart_on])).strftime("%Y-%m-%d")} 00:00:00' and delivery_date<='#{(Time.parse(session[:adminend_on])).strftime("%Y-%m-%d")} 23:59:59' and c.id=cs.campaign_id and (cs.impressions>0 or cs.clicks>0) group by campaign_id order by #{@adminreport_name=='amount_spent' ? 'amt_spent' : @adminreport_name} #{session[:order]}"
                             puts @strSql.inspect
                             @instCampaignData_pages, @instCampaignData = paginate_by_sql AdvCarrier,@strSql,session[:limit].to_i
                        end
                      ...
                      @adminXML_overall_report=@@objXmlBuilder.generate_query("campaigns_summaries-#{session[:duration]}",'Overall_Performance','campaign_id',session[:adminreport_name],session[:adminstart_on],session[:adminend_on],session[:order],session[:limit],'campaign_name')
                        if @adminXML_overall_report!=nil
                            # assign xml string to an hash and put it in a session variable for the Chart.
                            @adminxml_string_hash.merge!({:overall=>@adminXML_overall_report[:xml_string]})
                          end
                        @adminXML_summary_report=@@objXmlBuilder.generate_query("campaigns_summaries-#{session[:duration]}",'Campaigns_Performance','campaign_id',session[:adminreport_name],session[:adminstart_on],session[:adminend_on],session[:order],session[:limit],'campaign_name',((params[:page]!=nil) ? ((params[:page].to_i*session[:limit].to_i)-session[:limit].to_i) : 0 ))
                        if @adminXML_summary_report!=nil
                            # assign xml string to an hash and put it in a session variable for the Chart.
                            @adminxml_string_hash.merge!({:summary=>@adminXML_summary_report[:xml_string]})
                          end
                        session[:adminxml_string]=@adminxml_string_hash	
                        @strSql="select campaign_id, c.campaign_name, sum(clicks) as clicks , sum(impressions) as impressions, sum(cs.amount_spent) as amt_spent,((sum(clicks)/sum(impressions))*100) as ctr,((#{session[:adminreport_type]})/(select (#{session[:adminreport_type]}) as #{session[:adminreport_name]} from campaigns_summaries cs where delivery_date>='#{(Time.parse(session[:adminstart_on])).strftime("%Y-%m-%d")} 00:00:00' and delivery_date<='#{(Time.parse(session[:adminend_on])).strftime("%Y-%m-%d")} 23:59:59' and (cs.impressions>0 or cs.clicks>0)))*100 as percentage from campaigns_summaries cs,campaigns c where delivery_date>='#{(Time.parse(session[:adminstart_on])).strftime("%Y-%m-%d")} 00:00:00' and delivery_date<='#{(Time.parse(session[:adminend_on])).strftime("%Y-%m-%d")} 23:59:59' and c.id=cs.campaign_id and (cs.impressions>0 or cs.clicks>0) group by campaign_id order by #{session[:adminreport_name]=='amount_spent' ? 'amt_spent' : session[:adminreport_name]} #{session[:order]}"
                        @instCampaignData_pages, @instCampaignData = paginate_by_sql AdvCarrier,@strSql,session[:limit].to_i
                        #admincsv()
                      end
                else
