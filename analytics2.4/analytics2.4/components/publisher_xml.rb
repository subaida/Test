        ...
        def generate_query_data(*args) #generate_query starts here
          begin
                ...
                begin
                    ...
                    #~ sql_query+=" publisher_id =#{@pub_id}" if @pub_id!=nil 
                    #~ sql_query+=" and date>='2011-06-01' and date<='#{(Time.parse(@enddate)).strftime("%Y-%m-%d")}'" if @startdate!=nil and @startdate!="" and @enddate!=nil and @enddate!=""
                ...
                begin
                    ...
                    #~ puts "sql_query::::sql_query:::#{sql_query}"
                    #~ @recordset=@tablename.find_by_sql(sql_query) # fetch the recordset from table
                    @recordset=@tablename.show_data(@pub_id,type)
               rescue Exception=>e
              ...
              if @recordset.size !=0  #sub main If conditons starts here
                  for recordset in @recordset 
                       ...
                       if @report_type=='hourly_report'  
                              ...
                              count+=1
                              if (count.to_i<=5)
                                #~ puts "recordset[@column1]::::#{recordset[@column1]}:::::#{recordset.requests}:::::::#{@column1}:::#{args[8]}"
                             ...
                             count+=1
                              ...
                              generate_continentXML(recordset.country_code,recordset.impressions,@report_type,args[8]) 
                               ...
                               generate_linechartXML(recordset.requests,recordset.delivery_date,@startdate,@enddate,@report_type,@no_day,args[8])
        ...
        def generate_linechartXML(*args)
          ...
          def performance_summary(*args)
              begin
                  ...
                  @recordset=AdclientsSummary.find_by_sql("select (sum(impressions)+sum(fill_rate)) as sum_impressions,sum(clicks) as sum_clicks,((sum(clicks)/(sum(impressions)+sum(fill_rate)))*100)  as sum_ctr,sum(revenue) as sum_revenue,((sum(revenue)/(sum(impressions)+sum(fill_rate)))*1000) as sum_ecpm,pub_id from adclients_summaries  where pub_id =283 and ad_client_type='WAP' and delivery_date>='2011-01-06' and delivery_date<='2011-06-06' group by pub_id") 
                return @recordset
