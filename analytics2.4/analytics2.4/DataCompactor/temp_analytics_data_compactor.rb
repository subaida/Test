  ...
  def re_initialize(*args)
    ...
    def compress
      begin
        ...
        table_detail=['temp_campaigns_summaries','temp_adv_carriers','temp_adv_devicesproperties','temp_adclients_summaries','temp_pub_channels','temp_pub_carriers','temp_pub_urls','advertiser_summaries','publisher_summaries']
            ...
            @dbconn_selection = DBI.connect("DBI:MYSQL:#{@@analytics_read_db}:#{@@analytics_read_ip}", AnalyticsDBReadServer::USERNAME, AnalyticsDBReadServer::PASSWORD)
              begin
                ...
                if(get_lock == 1)
                  ...
                  tblname='temp_campaigns_summaries' if tblname=='advertiser_summaries'
                  tblname='temp_adclients_summaries' if tblname=='publisher_summaries'
                  ...
                  tblname='advertiser_summaries' if processing_table=='advertiser_summaries'
                  tblname='publisher_summaries' if processing_table=='publisher_summaries'
                else
                  tx_illegal = "true"
