                                        ...
                                        begin
                                            ...
                                            if((select_recovery_records.length > 0) and (select_records == "true"))  
                                                ...
                                                for row_table in select_recovery_records # Insert the selected rows one by one into the REDUCER table resides in analytics database. 
                                                    begin 
                                                        insert_query="insert into campaigns_summaries(metrics,ad_client_type,impressions,clicks,amount_spent,gross_profit,zestadz_revenue,handset,handset_model,unique_visitors,advertiser_id,campaign_id,ad_id,continent_code,country_code,delivery_hour,delivery_date) VALUES ('#{row_table['metrics']}','#{row_table['ad_client_type']}',#{row_table['impression']},#{row_table['click']},#{row_table['amt_spent']},#{row_table['grs_profit']},#{row_table['zest_revenue']},\"#{row_table['handset']}\",\"#{row_table['handset_model']}\",'#{row_table['uniq_visitors']}',#{row_table['advertiser_id']},#{row_table['campaign_id']},#{row_table['ad_id']},\"#{row_table['continent_code']}\",\"#{row_table['country_code']}\",#{row_table['delivery_hour']},'#{row_table['delivery_date']}')" if tblname.to_s=='reduce_campaigns_summaries'
                                                        insert_query="insert into adv_carriers(owner,requests,advertiser_id,campaign_id,ad_id,delivery_hour,delivery_date) VALUES (\"#{(row_table['owner']!=nil && row_table['owner']!='') ? row_table['owner'].to_s.gsub(/([,])/,'') : row_table['owner']}\",#{row_table['request']},#{row_table['advertiser_id']},#{row_table['campaign_id']},#{row_table['ad_id']},#{row_table['delivery_hour']},'#{row_table['delivery_date']}')" if tblname.to_s=='reduce_adv_carriers'
