...
class DeviceAtlas
  ...
  def getTreeFromString(string)
    ...
    #~ #puts "AFTER LOOP ------#{tree['p']}"
    #~ puts "VALUES OF V------------------------------START"
    #~ puts tree['v']
    #~ puts "VALUES OF V------------------------------END"
    
    #~ puts "VALUES OF $------------------------------START"
    #~ puts tree['$']
    #~ puts "VALUES OF $------------------------------END"
    
    #~ puts "VALUES OF p------------------------------START"
    #~ puts tree['p']
    #~ puts "VALUES OF p------------------------------END"
    
    #~ puts "VALUES OF t------------------------------START"
    #~ puts tree['t']['c'].keys
    #~ puts tree['t']['c']['v']
    #~ puts tree['t']['c']['v'].keys
    #~ puts "VALUES OF THE tree['t']['c']['v'].values"
    #~ puts tree['t']['c']['v'].values
    #~ puts "VALUES OF t------------------------------END"
    
    #~ tree['t'].each_pair { |key, value| puts "#{key.inspect} maps to #{value}"}
  ...
  def getProperty(tree, userAgent, property)
    @get_value = _getProperty(tree, userAgent, property, false)
    
    #puts "AFTER EXCEUTION OF CALL _getProperty THE SEEK PROPERTIES #{@get_value}"
  end
  ...
  def _getProperty(tree, userAgent, property, typedValue)    
    ...
    unmatched = ""
    self.found_properties = {}
    #puts "BEFORE CALL THE SEEK PROPERTIES #{userAgent.strip}----#{idProperties}-----#{sought}----#{matched}"
    _seekProperties(tree['t'], userAgent.strip, idProperties, sought, matched)
  ...
  def _seekProperties(node, string, properties, sought, matched)
    #puts "SEEKPORPERTIES METHOD node.has_key?('d') --------------#{node.has_key?('d')}"
    unmatched = string
    ...
    if node.has_key?('d')
      ...
      node['d'].each do |property,value|
        if (sought == nil || sought[property])
         ...
         # puts "THIS IS PORPERTIES OF THE VALUES ::::::#{properties}"
        end
    ...
    #puts "SEEKPORPERTIES METHOD node.has_key?('c') --------------#{node.has_key?('c')}"
    if node.has_key?('c')      
      (0..string.length+1).each do |c|    
        ...
        if node['c'].kind_of?(Hash) && node['c'][seek]
         ...
         # puts "THIS IS MATCHED VALUE------ #{matched}"
